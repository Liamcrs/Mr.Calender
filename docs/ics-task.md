...
Courses use `.course`, `importedUID` raw UID and reminderMinutes 15. IDs deterministic by UID + original occurrence so overrides preserve identity. This file is the historical parser-task brief; its earlier matching-UID reimport rule is superseded by the 2026-09-23 product decision: reimport replaces the selected timetable in full, so courses absent from the new file are removed while other timetables remain untouched.
