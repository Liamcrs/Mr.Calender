...
- **日历**：创建日程、指定时间、填写多行备注；标注中国传统节日和常见西方节日，并在日期上用蓝点标记节日、红点标记课程、黄点标记自定义日程；支持导入多个命名 `.ics` 课表，并可分别启用、隐藏、重新导入和删除；课程开始前 15 分钟通知。
- **健康**：通过健康问诊 Agent 对话整理作息、饮食和生活建议；支持手动记录运动，也可以读取 HealthKit/Apple Watch 的运动记录；显示今天和此前三个具体日期的 Apple 活动圆环，圆环需要 Health 授权和 Apple Watch 活动数据。
