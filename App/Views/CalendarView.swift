...
struct CalendarView: View {
    ...
    } }.navigationTitle("日历").toolbar { ToolbarItem(placement: .topBarLeading) { Button("导入课表") { showingImporter = true } }; ToolbarItem(placement: .topBarTrailing) { Button { showingAdd = true } label: { Image(systemName: "plus") } } }.fileImporter(isPresented: $showingImporter, allowedContentTypes: [.data], allowsMultipleSelection: false) { result in if case .success(let urls) = result, let url = urls.first, let text = try? String(contentsOf: url, encoding: .utf8) { store.importICS(text) } }.sheet(isPresented: $showingAdd) { AddEventView() } } }
}
